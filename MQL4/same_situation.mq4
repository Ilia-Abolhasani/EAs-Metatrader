...
double orderInterval = 0.25;	         // Distance between orders in points
int orderCount = 10;            // Number of orders to place on each side
double stopLossDistance = 2.5;     // Stop loss distance in points
double takeProfitDistance = 3;   // Take profit distance in points
double Lots = 0.01;             // Lot size for orders
double stdThreshold = 0.1;     // Standard deviation threshold for placing orders
int stdTicksNumbers = 50;
int removeOrderTime = 120; // seconds
int waitForOrderTime = 120; // seconds
...
void PlaceOrders()
{
    ...
    {                
        ...
        int result = OrderSend(_Symbol, OP_SELLLIMIT, Lots, orderPrice , 10, sl, tp, "Sell Order", 0, 0, clrRed);
        if (result > 0){        
        ...
        result = OrderSend(_Symbol, OP_BUYLIMIT, Lots, orderPrice , 10, sl, tp , "Buy Order", 0, 0, clrGreen);
		  if (result > 0){
